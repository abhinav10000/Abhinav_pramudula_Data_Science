...
x = logistic(train = train, valid = val)*100
...
library(e1071)
#library(klaR)
classifier_nb = naive_bayes(y = as.factor(train$Defaulter), x = train)
classifier_nb
...
pred1 = (ifelse(pred > 0.10, 1, 0))
pred1
cm2 = caret::confusionMatrix(as.factor(pred1),as.factor(val$Defaulter))
print(cm2)
